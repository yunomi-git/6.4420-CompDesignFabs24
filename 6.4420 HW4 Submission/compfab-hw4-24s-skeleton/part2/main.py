...
def set_boundary_conditions(mesh: TetMesh) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # plot_bc(vertices, bc_vertices=vertices[bc_mask], f_ext_vertices=vertices[f_ext_mask])
...
def solve_performance(stl_file: str, save_tet_mesh: str='') -> Tuple[float, float]:
    ...
    fem = StaticFEM(tet_mesh, material=material)               # <--
    ...
    u = fem.solve_linear(external_forces=f_ext, boundary_conditions=bc_mask)                 # <--

    #   4. Compute compliance (C = F^T * U)
    # HINT: use `a.ravel()` to flatten a 2D array
    compliance = (f_ext.ravel() * u.ravel()).sum()        # <--
...
def plot_bc(vertices, bc_vertices, f_ext_vertices):
    s = trimesh.Scene()
    bc = trimesh.points.PointCloud(vertices=bc_vertices,
                                   colors=np.array([0, 0, 255, 255]))
    f_ext = trimesh.points.PointCloud(vertices=f_ext_vertices,
                                   colors=np.array([0, 255, 0, 255]))
    mesh = trimesh.points.PointCloud(vertices=vertices,
                                   colors=np.array([255, 0, 0, 255]))
    s.add_geometry(mesh)
    s.add_geometry(bc)
    s.add_geometry(f_ext)
    s.show()

...
def pareto_front(points: np.ndarray) -> np.ndarray:
    ...
    points = points[np.lexsort((points.T[1, :], points.T[0, :]))]     # <--
    # Start from top left corner (small x)
    ...
    for i in range(points.shape[0]):
        ...
        # TODO: You code here.
        point_y = points[i, 1]
        # Next point is always to right of last pareto. It is not dominated if it is below (y <) the last point
        if point_y < pareto_y :       # <-- #

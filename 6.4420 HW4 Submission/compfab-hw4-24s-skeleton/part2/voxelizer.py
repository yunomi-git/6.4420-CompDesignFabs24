...
class Voxelizer:
        ...
        for ix in tqdm(range(nx)):            # <--
            for iy in range(ny):        # <--
                for iz in range(nz):    # <--
                    ...
                    voxel_center = voxel_bottom_left + np.array([ix * dx, iy * dx, iz * dx])
                    ...
                    locations = single_ray_mesh_intersection(mesh=mesh, ray_origin=ray_origin, ray_direction=ray_direction)
                    ...
                    if len(locations) % 2 == 1: # odd intersections
                        voxels[ix, iy, iz] = 1
                print(f'Completed y layer {iy + 1} / {ny}')
            print(f'Completed x layer {ix + 1} / {nx}')
    ...
    def run_accelerated(self, check_result: bool=True) -> float:
        ...
        intersections = parallel_ray_mesh_intersection(mesh=mesh, ray_origins=ray_origins, ray_direction=ray_direction,
                                   origins_outside=True, perturbation=0.01)
        ...
        for ix in range(nx):        # <--
            for iy in range(ny):    # <--
                ...
                distances = np.array(intersections[ix * ny + iy]) # TODO correct?
                ...
                if len(distances):
                    ...
                    for i in range(len(lower_indices)):        # <-- for loop
                        voxels[ix, iy, lower_indices[i]:upper_indices[i]] = 1            # <-- loop body
...
#     name = "sphere"
#     path = paths.HOME_PATH + "stls/" + name + ".stl"
#     voxelizer = Voxelizer(path, voxel_size=1)
#     voxelizer.run_accelerated()
#     voxelizer.save_mesh(paths.HOME_PATH + "stls/" + name + "_voxel_accel.stl")
#     voxelizer.save_to_txt_file(paths.HOME_PATH + "stls/" + name + "_voxel.txt")